...
import java.util.stream.Collectors;
    ...
    public class App {
        ...
        static void levelTwoOnlyForTheDareDevils(){
            ...
            // Complex Questions:
            System.out.println("\n\tLevel Two Only For The Dare Devils:");

            // 1. Find the top 3 highest-rated movies for each genre.
            // Code your solution here
            List <String> top3Movies =Movie.getMovies().stream().filter(n-> n.getGenre().equals(n.getGenre()))
            .sorted((a,b)->Double.compare(a.getRating(), b.getRating())).limit(3).map(Movie::getTitle).collect(Collectors.toList());

             System.out.println("\n1. top 3 movies:"+top3Movies);


            // 2. Get a map of the average rating of movies per genre.
            // Code your solution here
            Map<String,Double> averageRatingPerGenre = Movie.getMovies().stream().collect(Collectors.groupingBy(Movie::getGenre,Collectors.averagingDouble(Movie::getRating)));
             System.out.println("\n2. Average Rating: "+averageRatingPerGenre);

            // 3. Find the movie with the maximum duration in each genre.
            // Code your solution here
            List<Movie> maxDurationPerGenre = Movie.getMovies().stream()
            .collect(Collectors.groupingBy(Movie::getGenre)).entrySet().stream()
            .map(entry -> entry.getValue().stream()
                .max(Comparator.comparingInt(Movie::getDuration))
                .orElse(null)).filter(Objects::nonNull).collect(Collectors.toList());
        
                  System.out.println("\n3. Movies with maximum duration per genre: " + maxDurationPerGenre);
        
            // 4. Get a list of movies that have both 'Action' and 'Drama' as genres.
            // Code your solution here
            List<Movie> actionAndDramaMovies = Movie.getMovies().stream()
            .filter(n -> n.getGenre().contains("Action") && n.getGenre().contains("Drama"))
            .collect(Collectors.toList());
                if(!actionAndDramaMovies.isEmpty()){
                    System.out.println("\n4. Movies with both 'Action' and 'Drama' genres: " + actionAndDramaMovies);
                }
                else{
                    System.out.println("\n4. NO RESULT FOUND");
                }
            ...
            // Code your solution here
            Optional<Movie> secondHighest = Movie.getMovies().stream().sorted(Comparator.comparingDouble(Movie::getRating).reversed()).skip(1).findFirst();
             System.out.println("\n5. Second Highest Movie: "+secondHighest);

            // 6. Get a list of movies whose title starts with the letter 'A'.
            // Code your solution here
            List<String> startsWithA = Movie.getMovies().stream().filter(n-> n.getTitle().startsWith("A") || n.getTitle().startsWith("a")).map(Movie::getTitle).collect(Collectors.toList());
             System.out.println("\n6. Title Starts with A: "+startsWithA);

            // 7. Get a map of movie titles and their respective durations in hours and minutes.
            // Code your solution here
            Map<String, String> movieDurationMap = Movie.getMovies().stream()
            .collect(Collectors.toMap(Movie::getTitle, n -> {
                    int duration = n.getDuration();
                    return duration / 60 + "h " + duration % 60 + "m";
                }));
        
         System.out.println("\n7. Movie titles and durations (hours and minutes): " + movieDurationMap);
        

            // 8. Find all movies released in the last 10 years.
            // Code your solution here
            List<String> releasedInTenYears = Movie.getMovies().stream().filter(n-> n.getReleaseYear()>=(Year.now().getValue()-10)).map(Movie::getTitle).collect(Collectors.toList()); 
             System.out.println("\n8. Movies released on last 10 years: "+releasedInTenYears);
